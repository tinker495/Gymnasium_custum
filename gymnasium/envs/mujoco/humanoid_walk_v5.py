...
class HumanoidEnv(MujocoEnv, utils.EzPickle):
    ...
    **Note:** There is a bug in the `Humanoid-v4` environment that causes *contact_cost* to always be 0.
    ...
    ## Starting State
    The initial position state is $[0.0, 0.0, 1.4, 1.0, 0.0, ... 0.0] + \mathcal{U}_{[-reset\_noise_scale \times I_{24}, reset\_noise_scale \times I_{24}]}$.
    The initial velocity state is $\mathcal{U}_{[-reset\_noise_scale \times I_{23}, reset\_noise_scale \times I_{23}]}$.
    ...
    | `velocity_deviation_penalty`                 | **float** | `1.0`            | Weight for velocity deviation penalty in the reward function (see `Rewards` section)                                                                                                                        |
    ...
    * v0: Initial versions release
    ...
    def __init__(
        ...
        velocity_deviation_penalty: float = 1.0,
        **kwargs,
    ):
        ...
        self._velocity_deviation_penalty = velocity_deviation_penalty

        self._target_velocity = 0.0  # Will be set during reset
